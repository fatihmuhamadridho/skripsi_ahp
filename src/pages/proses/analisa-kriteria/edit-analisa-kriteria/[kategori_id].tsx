...
const TambahAnalisaKriteria = () => {
  ...
  useEffect(() => {
    if (detailKategori && isRender) {
      setAnalisaKonsistensi(detailKategori?.AnalisaKriteria);
      setIsRender(false);
    }
  }, [detailKategori, isRender]);

  const handleEditData = async (values: any) => {
    const payload = { ...values, matriks_perbandingan: undefined };
    try {
      const response = await AnalisaKriteriaService.putAnalisaKriteria(
        {
          ...payload,
          ...{
            ...analisaKonsistensi,
            analisa_kriteria_id: undefined,
  ...
  return (
    <DefaultTemplate title="TambahAnalisaKriteria">
      <Formik
        enableReinitialize={isRender}
        initialValues={{
          name: detailKategori?.name || "",
          periode: detailKategori?.periode || new Date(),
          kriteria: detailKategori?.KategoriKriteria?.map((item: any) => {
          ...
          matriks_perbandingan: detailKategori?.AnalisaKriteria
            ?.matriks_perbandingan || [
            {
              kriteria_id: 0,
